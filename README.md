...
The production Google Drive should include the documents `confirm order readme.txt` and `PO Template 模板` in the root directory as well as the folder `进行生产任务 Current Order`.  The `PO Template 模板` template file must be a Google Sheet which contains the named cell range `orderFileId`.
